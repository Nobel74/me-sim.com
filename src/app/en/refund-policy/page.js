...
export default function EnglishRefundPolicyPage() {
  ...
  useEffect(() => {
    // Fetch refund policy page in English from WordPress REST API
    const fetchWpPage = async () => {
      try {
        setLoading(true);
        const res = await fetch('https://me-sim.com/wp-json/wp/v2/pages?slug=refund-policy');
        ...
        console.error('Error fetching EN WP Refund Policy:', err);
      }

      // Fallback structure with structured HTML hierarchy
      setPageData({
        title: { rendered: 'Refund Policy' },
        content: {
          rendered: `
    ...
    <div className="container-naked max-w-4xl font-sans">
      ...
      <div className="bg-white rounded-3xl border border-zinc-200 p-6 sm:p-10 md:p-12 shadow-xl">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-black mb-6 tracking-tight">
          {pageData?.title?.rendered || 'Refund Policy'}
        </h1>

        {loading ? (
          <div className="py-12 text-center text-zinc-400 font-medium animate-pulse">
            Loading Refund Policy from WordPress...
          </div>
        ) : (
          <div
