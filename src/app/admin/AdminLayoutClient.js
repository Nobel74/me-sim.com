...
export default function AdminLayoutClient({ children }) {
  ...
  useEffect(() => {
    ...
    if (pathname === '/admin/precios') {
      setActiveTab('precios');
    } else if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      const rawTab = params.get('tab') || localStorage.getItem('mesim_admin_tab') || 'dashboard';
      const tabParam = ['company', 'partners'].includes(rawTab) ? 'settings' : rawTab;
      if (['dashboard', 'orders', 'precios', 'settings'].includes(tabParam)) {
    ...
    const handleTabEvent = (e) => {
      const rawTab = e.detail;
      const tab = ['company', 'partners'].includes(rawTab) ? 'settings' : rawTab;
  ...
  const navItems = [
    {
      ...
      shortLabel: { es: 'Finanzas', en: 'Finance' },
      icon: (
    ...
    },
    {
      ...
      shortLabel: { es: 'Clientes', en: 'Customers' },
      icon: (
        <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      shortLabel: { es: 'Configuración', en: 'Settings' },
      icon: (
        <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
