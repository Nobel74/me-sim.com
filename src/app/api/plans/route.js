...
import { ALL_WORLD_COUNTRIES, COUNTRY_NAMES, REGION_NAMES } from '../../../lib/i18n.js';
import { getPricingRules, computePlanPricing, mapIsoToRegion } from '../../../lib/pricingRules.js';
import { getRegionDefinition, isRegionalSlug, isPlanInRegion, REGION_MAPPING } from '../../../lib/regionMapping.js';
...
const applyMarkup = (plansList) => {
  ...
  return plansList.map((p) => {
    ...
    return {
      ...
      profitNetEur: pricing.profitNetEur,
      profitNetPct: pricing.profitNetPct,
    };
  });
};
...
export async function GET(request) {
  ...
  const queryTarget = country || region || '';
  const regionDef = getRegionDefinition(queryTarget);
  const isRegionQuery = !!regionDef;
  const canonicalRegion = isRegionQuery ? regionDef.canonicalSlug : null;
  ...
  try {
    ...
    if (response && response.ok) {
      ...
      if (rawPlans && rawPlans.length > 0) {
        ...
        if (isRegionQuery) {
          // Filtrado estricto por matriz de mapeo regional (regionCode / keywords)
          const matched = rawPlans.filter((p) => isPlanInRegion(p, queryTarget));
          mappedPlans = matched.map((p) => mapSinglePlan(p, queryTarget, canonicalRegion, true, regionDef.nameEs));
        } else if (queryTarget) {
          const matched = rawPlans.filter((p) => {
            const pIso = (p.country_code || p.iso || p.isoCode || p.location || '').toLowerCase().trim();
            const pReg = (p.region || '').toLowerCase().trim();
            return pIso === queryTarget || pReg === queryTarget;
          });
          mappedPlans = matched.map((p) => mapSinglePlan(p, null, null, false));
        } else {
          // Catálogo global: mapear planes de países individuales
        ...
        if (mappedPlans.length > 0) {
          // Deduplicate live api plans keeping best wholesale cost
          const uniqueMap = new Map();
  ...
  if (targetCode && !countryMeta.some((c) => c.iso === targetCode) && !regionMeta.some((r) => r.iso === targetCode)) {
    ...
    const selectedTiers = isRegionFallback ? regionTiers : countryTiers;
